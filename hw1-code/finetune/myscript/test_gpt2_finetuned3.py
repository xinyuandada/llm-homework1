...
peft_model_path = "/data/gpt2-alpaca-outputs-5_27-try1/final_checkpoint" 

tokenizer = AutoTokenizer.from_pretrained(peft_model_path,padding_side="left", pad_token="<|endoftext|>")
...
# 加载基础模型和 PEFT 适配器
base_model = AutoModelForCausalLM.from_pretrained(model_name, config=model_config).to(device)
finetuned_model = PeftModel.from_pretrained(base_model, peft_model_path).to(device)
...
def generate_response(model_to_test, instruction, input_text=None):
    ...
    with torch.no_grad():
        outputs = model_to_test.generate(
            ...
            pad_token_id=tokenizer.eos_token_id, # 确保设置了 pad_token_id
...
# 测试问题 1
q1 = "Where is the capital of United States?"
print(f"Q: {q1}")
print(f"A: {generate_response(finetuned_model, q1)}")

# 测试问题 2
q2 = "Write a short story about a robot."
print(f"\nQ: {q2}")
print(f"A: {generate_response(finetuned_model, q2)}")

# 测试问题 3 (带输入的) - 总结放羊孩子的故事
...
print(f"\nQ: {q3}")
print(f"Input: {i3}")
print(f"A: {generate_response(finetuned_model, q3, i3)}")

# 测试问题 4 --- 测试集原问题
q4 = "Who is the world's most famous painter?"
print(f"\nQ: {q4}")
print(f"A: {generate_response(finetuned_model, q4)}")